...
function updateSections() {
    ...
    document.querySelectorAll(".word-unit").forEach(unit => {
        ...
        if (search) {
            visible = word.spell.toLowerCase().includes(search);
        }

        if (state.viewDontKnowOnly) {
            visible = visible && word.status === "dontknow";
        }
        ...
        if (search && spell.toLowerCase().includes(search)) {
            ...
            spellEl.innerHTML =
                spell.replace(regex, `<span class="search-highlight">$1</span>`);
...
function updateVisibleBorders() {
    ...
    document.querySelectorAll(".sect-words-group").forEach(group => {
        ...
        const visibleWords = [...group.querySelectorAll(".word-unit")]
