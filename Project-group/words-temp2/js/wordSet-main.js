document.addEventListener("DOMContentLoaded", async () => {
    ...
    const wordsSectScrollArea =
        document.getElementById("words-sect-scroll-area");

    const searchInput =
        document.getElementById("search-word-input");

    const searchDelBtn =
        document.getElementById("search-del-btn");

    const gotoInput =
        document.getElementById("gotonum-input");

    const gotoArrow =
        document.getElementById("gotonum-arrow");

    const viewMeanToggle =
        document.getElementById("view-mean-toggle-switch");

    const sectSepr =
        document.getElementById("sect-sepr");

    const sectSeprGroup =
        document.getElementById("sect-sepr-group");

    const sectSeprSetup =
        document.getElementById("sect-sepr-setup");

    const sectSeprWhole =
        document.getElementById("sect-sepr-whole");

    const sectSeprInput =
        document.getElementById("sect-sepr-input");
    ...
    function renderSections() {
        ...
        if (searchText) {
            ...
            words = words.filter(word =>
                word.spell.toLowerCase().includes(searchText)
            );
        ...
        words.forEach(word => {
            ...
            if (!sectionMap[word.section]) {
                sectionMap[word.section] = [];
            }
        ...
        /* ⭐ 검색하면 관련 section 자동 open */
        if (searchText) {

            document.querySelectorAll(".words-sect")
                .forEach(sect => {

                    if (sect.querySelector(".word-unit")) {
    ...
    function createSectionElement(sectionNum, words, searchText) {
        ...
        words.forEach(word => {
            group.appendChild(
                createWordElement(word, searchText)
            );
        });
    ...
    function gotoNumber() {
        ...
        const target =
            document.querySelector(
                `.word-unit[data-word-id="${num}"]`
            );

        if (target) {

            const sect =
                target.closest(".words-sect");
