...
window.addEventListener("load", () => {
    ...
    leftInner.scrollTop = leftInner.scrollHeight;
    rightInner.scrollTop = rightInner.scrollHeight;
...
new ResizeObserver(resizeKeyboard).observe(ipaKeyboardInner);
...
function buildKeyboard(){
    ...
    IPA_ROWS.forEach((rowData,rowIndex)=>{
        ...
        rowData.forEach(key=>{
            ...
            btn.addEventListener("pointerup",stopDeleteHold);
            btn.addEventListener("pointerleave",stopDeleteHold);
            btn.addEventListener("pointercancel",stopDeleteHold);
...
function resizeKeyboard(){
    ...
    const baseCols=10;
    const keyWidth=(innerWidth-gap*(baseCols-1))/baseCols;

    document.querySelectorAll(".ipa-key").forEach(btn=>{
...
let deleteDelayTimer = null;
let deleteRepeatTimer = null;

const DELETE_INITIAL_DELAY = 450;
const DELETE_REPEAT_RATE = 40;

function startDeleteHold(){

    stopDeleteHold();

    deleteDelayTimer = setTimeout(()=>{
...
function pressKey(key){
    ...
    if(key==="<"){
        activeInput.setSelectionRange(Math.max(0,start-1),Math.max(0,start-1));
        return;
    }

    if(key===">"){
        activeInput.setSelectionRange(Math.min(len,start+1),Math.min(len,start+1));
        return;
    }
...
function updateKeyboardPadding(){
    ...
    const h = ipaKeyboard.offsetHeight;
