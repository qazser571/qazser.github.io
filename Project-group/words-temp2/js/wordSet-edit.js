...
/* ===================== EXPORT SPELL TXT (etest-btn) ===================== */

const etestBtn = document.getElementById("etest-btn");

etestBtn?.addEventListener("click", () => {

    collectInputs();

    const spells = tempSet.words.map(w => w.spelling ?? "");
    const text = spells.join("/");
    ...
    a.href = url;
    a.download = "spelling.txt";
    a.click();

    URL.revokeObjectURL(url);
});

/* ===================== IMPORT PHONETIC TXT (ptest-btn) ===================== */

const ptestBtn = document.getElementById("ptest-btn");
...
ptestBtn?.addEventListener("click", () => {
    ...
    input.addEventListener("change", async () => {
        ...
        const phonList = text.split("/");

        tempSet.words.forEach((word, i) => {
            word.phonetic = phonList[i] ?? "";
        });
